...
| Track 1 / A2 (veilgremlin: real TLS client, real Claude Code session) | **vg-proxy's own side done and tested, 2026-09-14; live-run proof blocked on a real, external finding — RISK-0014, not this milestone's own scope.** | veilgremlin `agent/claude/a2-real-tls-client` (commit `7c0f3ac`), intent `INT-2026-09-13-001` (`in_progress`, two user-approved amendments). Built and tested: real `rustls`/`rustls-native-certs` TLS client with genuine WebPKI cert/hostname verification (`tests/tls_upstream.rs`, 3 tests against a real local TLS server); a minimal SSE demask path pulled forward from M6 after the live-run proof found the real `claude` CLI always sends `stream: true` (`stream_demask.rs`, a chunk-boundary-split-placeholder regression proven). `cargo build/clippy/fmt/test --locked`, `cargo deny check`, `cargo audit` all clean. **Blocked, not a code defect:** the real `claude` CLI redacts its own internal billing header (`x-anthropic-billing-header`) to `[REDACTED:SECRET]` whenever `ANTHROPIC_BASE_URL` points away from the default Anthropic endpoint, and the real Anthropic API rejects that redacted value outright — confirmed not specific to vg-proxy (fires against any custom `ANTHROPIC_BASE_URL`, the exact mechanism this beta's condition 1 and this whole plan's M5/A2 framing depend on). Filed as `veilgremlin` `RISK-0014` (Critical, Open) — needs a real owner and decision this repo cannot make unilaterally before condition 1 can be proven with a real, unmodified CLI session. |
