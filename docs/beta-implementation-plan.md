...
| Track 1 / A2 (veilgremlin: real TLS client, real Claude Code session) | **Done and proven end to end, 2026-09-14.** | veilgremlin `agent/claude/a2-real-tls-client` (commit `7c0f3ac` + follow-up), intent `INT-2026-09-13-001` (two user-approved amendments). Built and tested: real `rustls`/`rustls-native-certs` TLS client with genuine WebPKI cert/hostname verification (`tests/tls_upstream.rs`, 3 tests against a real local TLS server); a minimal SSE demask path pulled forward from M6 after the live-run proof found the real `claude` CLI always sends `stream: true` (`stream_demask.rs`, a chunk-boundary-split-placeholder regression proven). `cargo build/clippy/fmt/test --locked`, `cargo deny check`, `cargo audit` all clean. The live-run proof then found and closed a real, external blocker same day: the real `claude` CLI redacts its own billing-attribution header against any custom `ANTHROPIC_BASE_URL`, which the real API rejects — fixed with `CLAUDE_CODE_ATTRIBUTION_HEADER=0` (Anthropic's own documented gateway-compatibility setting; `veilgremlin`'s `vg run` already injects it, just never previously exercised in this proof). `scripts/a2-live-proof.sh` now passes end to end against the real, unmodified CLI: real TLS, real streaming, real masking/demasking, real subscription auth. `RISK-0014`/`RISK-0008` both closed. **Still open, not solved by this closure:** the broader "sit in front of multiple harnesses" question (Codex's own independent recommendation: a process-scoped explicit forward proxy + generic transport/per-provider-codec architecture, not system-wide TLS interception) is real, unstarted design work for a later item, not this milestone's own scope. |
| Track 1 remainder (A3-A8) and Track 2 (Phase B/C/D/E) | **Not started** | Unblocked per §5's dependency graph; A2 is the only Track 1 item picked up so far this pass. |
