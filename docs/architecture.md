...
| veil-enrol → veil-custodian (`POST /devices` enrolment, `.../certificates/renew`, `.../signing-keys`, `.../signing-keys/{key_ref}/renew`) | **Built, locally proven.** veil-enrol (PR #2, merged to `main` 2026-09-04) is the real, first, and only caller of these endpoints anywhere in the family, per ADR-D/ADR-N's own design. The fourth (`renew-signing-key`, XREPO-003/ADR-T) added 2026-09-06. `scripts/dev-e2e.sh` proves the full loop against a real local veil-custodian instance, including issued-certificate profile verification and, since 2026-09-06, a real signing-key renewal with the predecessor's `superseded_by` confirmed via a live `GET` as veil-observatory. The on-device CSR-generation/install story that used to be missing here now exists (`XREPO-009`, closed 2026-09-12): veilgremlin's `vg enrol request-csr`/`install-cert` generates the keypair and CSR device-side, feeds it through this exact veil-enrol caller, and installs the resulting certificate to the real OS keychain — live-proven end to end in `veil-demo/scripts/xrepo-009-device-credential-install-proof.sh`. Still not a full production integration: no MDM actually invokes either `veil-enrol` or `vg enrol` today — a human operator runs both by hand — and there is no CA trust-anchor *distribution* mechanism yet (`XREPO-010`). |
...
4. ~~Wire one real observatory→custodian call.~~ — **done, 2026-09-06 (`XREPO-001`
   ...
   `attestation/status` and `/certificates/crl` were tracked separately as `XREPO-006`
   since this step only ever asked for one real call — also now done, 2026-09-06 (ADR-0020,
   live-proven in the same script's Steps 11-13). All three `Role::Observatory` grants now
   have real callers. The attestation consumer's own live-run proof then surfaced that
   ...
   sighting can now reach `accepted`. `XREPO-009` is now also **closed, 2026-09-12**
   (veilgremlin ADR-017, live-proven in
   `veil-demo/scripts/xrepo-009-device-credential-install-proof.sh`) — `vg enrol`
   writes a real signing credential to the real OS keychain, and organic, zero-seam
   traffic can now reach `accepted` with a real `device_ref`, not just a seamed proof.
   Six narrower follow-on items (`XREPO-010`-`XREPO-015`) filed on that closure rather
   than absorbed into it — see `.hekton/cross-repo-deps.yaml`.
