...
| XREPO-004 | veilgremlin, veil-custodian, veil-observatory | Open | veilgremlin's raw `r\|\|s` (not DER) signature-encoding decision asks for sign-off from the other two — veil-custodian's own backlog records the ask; veil-observatory's does not mention it at all. |
