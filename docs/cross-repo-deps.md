...
| XREPO-008 | veil-observatory, veilgremlin, veil-custodian | Open | veil-observatory has no real ECDSA verification path — an ECDSA-signed record (the only kind that ever carries a real `device_ref`) can never reach disposition `accepted`, so no consumer can mint a finding from real, organic ECDSA traffic. Filed on `XREPO-007`'s closure as its own Phase 2 item. Raw `r\|\|s` wire encoding already settled (`XREPO-004`). |
