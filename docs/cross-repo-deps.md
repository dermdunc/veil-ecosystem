...
| XREPO-006 | veil-observatory, veil-custodian | Open | veil-observatory has no consumer for `/v1/attestation/status` or `/v1/certificates/crl` — no stub, no fixture, no `FindingEngine`/`Correlator` seam wants this data yet. Split out from `XREPO-001` on closure: real design-from-scratch work, not a fixture swap. `tests/fitness/test_architecture.py`'s ADR-0011 exemption means any new caller must live in `custodian.py`, reachable only from `cli.py` — a CRL cache consulted inline by the pipeline is architecturally excluded. |
